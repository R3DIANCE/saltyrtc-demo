...
import org.saltyrtc.tasks.webrtc.transport.ISignalingTransportHandler;
import org.saltyrtc.tasks.webrtc.transport.ISignalingTransportLink;
...
public class PeerConnectionHelper {
	...
	private void onIceCandidatesReceived(@NonNull final Candidate[] candidates) {
		for (@Nullable final Candidate candidate : candidates) {
		    ...
		    final IceCandidate iceCandidate = new IceCandidate(
		        candidate.getSdpMid(), candidate.getSdpMLineIndex(), candidate.getCandidate());
            Log.d(TAG, "New remote candidate: " + candidate.getCandidate());
    ...
    private void prepareHandover() {
        // Get transport link
        final ISignalingTransportLink link = this.task.getTransportLink();
